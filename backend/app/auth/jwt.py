...
def create_access_token(data: dict, scope: str = "user", audience: Optional[str] = DEFAULT_AUDIENCE) -> str:
    """
    ...
    """
    data["scope"] = scope
    return create_token(data, timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES), "access", audience)
...
def create_refresh_token(data: dict, audience: Optional[str] = DEFAULT_AUDIENCE) -> str:
    ...
    Returns:
        str: JWT de refresh.
    """
    return create_token(data, timedelta(days=REFRESH_TOKEN_EXPIRE_DAYS), "refresh", audience)

def decode_token(token: str, expected_type: str = "access", audience: Optional[str] = DEFAULT_AUDIENCE) -> Optional[dict]:
    """
